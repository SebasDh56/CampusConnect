...
class PaymentConfirmedConsumer:
    ...
    def start(self) -> None:
        ...
        self._thread = threading.Thread(target=self._run, name="academic-payment-consumer", daemon=True)
        self._thread.start()
        ...
        while not self._stopping.is_set():
            try:
                ...
                channel.basic_consume(
                    queue=settings.rabbitmq_payment_queue,
                    on_message_callback=self._handle_message,
                    auto_ack=False,
                )
                logger.info("academic_payment_consumer_started queue=%s", settings.rabbitmq_payment_queue)
                channel.start_consuming()
            except Exception:
                if not self._stopping.is_set():
                    logger.exception("academic_payment_consumer_connection_failed")
                    time.sleep(5)
    ...
    def _handle_message(
        ...
        if (
            not event_id
            or event_type != "PaymentConfirmed"
            or routing_key != PAYMENT_CONFIRMED_ROUTING_KEY
            or not student_id
            or not payment_id
        ):
            logger.error(
        ...
        try:
            ...
            student = crud.mark_student_payment_confirmed(db, student_id, payment_id)
            if student is None:
                self._record_failure(db, event, routing_key, f"student {student_id} not found")
                logger.error(
                    "event_processing_failed eventId=%s eventType=%s correlationId=%s routingKey=%s status=failed reason=student_not_found",
                    event_id,
                    event_type,
                    correlation_id,
                    routing_key,
            ...
            logger.info(
                "student_financial_status_updated eventId=%s eventType=%s correlationId=%s routingKey=%s studentId=%s paymentId=%s status=processed",
                event_id,
                event_type,
                correlation_id,
                routing_key,
            ...
            db.rollback()
            self._record_failure(SessionLocal(), event, routing_key, str(exc))
            logger.exception(
    ...
    def _record_failure(self, db: Session, event: dict, routing_key: str, error: str) -> None:
        try:
            event_id = event.get("eventId")
