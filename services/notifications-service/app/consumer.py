...
from app.handlers import handle_incident_reported, handle_payment_confirmed, handle_student_enrolled
from app.models import Notification, ProcessedEvent

logger = logging.getLogger(__name__)

HANDLERS = {
    "student.enrolled": handle_student_enrolled,
    "payment.confirmed": handle_payment_confirmed,
    "incident.reported": handle_incident_reported,
}
...
class NotificationsConsumer:
    ...
    def start(self) -> None:
        ...
        self._thread = threading.Thread(target=self._run, name="notifications-consumer", daemon=True)
        self._thread.start()
    ...
    def _run(self) -> None:
        while not self._stopping.is_set():
            try:
                ...
                logger.info("notifications_consumer_started queue=%s", settings.rabbitmq_queue)
                channel.start_consuming()
            except Exception:
                if not self._stopping.is_set():
                    logger.exception("notifications_consumer_connection_failed")
                    time.sleep(5)
    ...
    def _handle_message(
        ...
        try:
            ...
            if handler is None:
                self._record_failure(db, event, routing_key, "unsupported routing key")
                channel.basic_nack(delivery_tag=method.delivery_tag, requeue=False)
                return
            ...
            notification: Notification = handler(event)
